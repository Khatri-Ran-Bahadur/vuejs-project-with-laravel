...
('$pid','$title','$msg','$todaydate','$cid','$sid','$bid','$acyear','1','parent')");
	 if($sql){
		 	 header("Location:feedback_admin.php?msg=succ");		 	 
	 }else{
		 header("Location:feedback_admin.php?msg=err");
	 }
    ...
    <div class="content">
        <?php include("includes/head.php");?>
         <?php
				...
				?>  
            <h1 class="page-title">Admin - Feedback</h1>
        </div>
...
<div class="row-fluid" style="min-height:320px">
   <div class="block span12">
        <div class="block-heading">
            ...
            <a href="#widget2container" data-toggle="collapse"> Admin -  Feedback</a>
        </div>
        <div id="widget2container" class="block-body collapse in">
            ...
            <!--<thead>
              ...
              <tbody>
              <?php $qury1=mysql_query("SELECT * FROM afeedback WHERE c_id=$cid AND s_id=$sid AND p_id=$pid AND ay_id=$acyear AND b_id=$bid"); 
						$totasl =0;
					while($subject3=mysql_fetch_array($qury1))
					{ 
					$afid=$subject3['af_id'];
					 $sql=mysql_query("UPDATE afeedback SET status='0' WHERE af_id='$afid' AND send='admin'");
					 
					$send=$subject3['send'];
					?>
                  <tr style="border:1px solid #C1C0C0;">
                      <td width="150px;">
                          <?php if($send=="parent"){ echo "<strong> You<br></strong><span style=' font-size:12px'>".$subject3['date']."</span> "; } else { echo "<strong> Admin <br></strong><span style=' font-size:12px'>".$subject3['date']."</span> ";}?>                        
                      </td>
